...
layer_sizes = [img_size, 10, n_cats]
...
for i in range(epochs):
	...
	H = forward_propagate(flattened_train_x, W)
	pred_y = H[-1]
	...
	# backpropagation
	for (w,h,out) in zip(reversed(W), list(reversed(H[:-1]))+[flattened_train_x], reversed(H)):
		# error on previous layer * derivative of activation (sigmoid)
		gradient = err * out * (1.0 - out)

		# apply gradient with step size to next layer output
		delta = -eps * numpy.dot(gradient.T, h)
